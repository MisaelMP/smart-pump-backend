...
const generateRefreshToken = (config: AuthConfig, user: User, tokenVersion: number = 1): string => {
...
// Verify access token
const verifyAccessToken = (config: AuthConfig, token: string): TokenPayload | null => {
	try {
...
const verifyRefreshToken = (config: AuthConfig, token: string): RefreshTokenPayload | null => {
	try {
...
const generateTokens = (config: AuthConfig, user: User, tokenVersion: number = 1) => {
...
const decodeTokenWithoutVerification = (token: string): TokenPayload | RefreshTokenPayload | null => {
	try {
...
export const authService = {
	...
	generateRefreshToken: (user: User, tokenVersion?: number) => generateRefreshToken(authConfig, user, tokenVersion),
	...
	generateTokens: (user: User, tokenVersion?: number) => generateTokens(authConfig, user, tokenVersion),
